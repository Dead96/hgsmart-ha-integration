...
from .const import MAX_PORTIONS, MIN_PORTIONS
...
class HGSmartPortionSelect(HGSmartDeviceEntity, SelectEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if last_state is not None and last_state.state in PORTION_OPTIONS:
            self.coordinator.set_portions(int(last_state.state))

    @property
    def current_option(self) -> str:
        return str(self.coordinator.get_portions())

    async def async_select_option(self, option: str) -> None:
        self.coordinator.set_portions(int(option))
